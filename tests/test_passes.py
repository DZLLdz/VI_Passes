...
def test_send_data(test_client, sample_user):
    ...
    # Убедитесь, что ответ можно распарсить как JSON
    try:
        response_json = response.get_json()
    except ValueError:
        pytest.fail("Response data is not valid JSON")

    # Проверка наличия ключа 'message' в JSON
    assert 'message' in response_json

#
# def test_get_pass(client, sample_pass):
#     response = client.get(f'/api/submitData/{sample_pass.id}')
#     assert response.status_code == 200
#     data = json.loads(response.data)
#     assert data['data']['beautyTitle'] == sample_pass.beautyTitle
#
#
# def test_get_passes_for_mail(client, sample_user, sample_pass):
#     response = client.get(f'/api/passes/{sample_user.email}')
#     assert response.status_code == 200
#     data = json.loads(response.data)
...
#         }
#     }
#     response = client.post('/api/sendData', json=data)
#     assert response.status_code == 200
#     response_data = response.get_json()
#     assert response_data['message'] == 'Data submitted successfully'
#
#
# def test_patch_pass(client, sample_pass):
#     data = {
#         "data": {
...
#         }
#     }
#     response = client.patch(f'/api/submitData/{sample_pass.id}', json=data)
#     assert response.status_code == 200
